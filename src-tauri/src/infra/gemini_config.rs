...
#[serde(rename_all = "camelCase")]
pub struct GeminiSettingsJsonState {
    pub config_path: String,
    pub exists: bool,
    pub json: String,
}
...
#[derive(Debug, Clone, Serialize, specta::Type)]
#[serde(rename_all = "camelCase")]
pub struct GeminiSettingsJsonValidationError {
    pub message: String,
    pub line: Option<u32>,
...
#[derive(Debug, Clone, Serialize, specta::Type)]
#[serde(rename_all = "camelCase")]
pub struct GeminiSettingsJsonValidationResult {
    pub ok: bool,
    pub error: Option<GeminiSettingsJsonValidationError>,
}
...
fn json_validation_error(message: impl Into<String>) -> GeminiSettingsJsonValidationResult {
    GeminiSettingsJsonValidationResult {
        ok: false,
        error: Some(GeminiSettingsJsonValidationError {
...
fn json_parse_validation_error(error: serde_json::Error) -> GeminiSettingsJsonValidationResult {
    GeminiSettingsJsonValidationResult {
        ok: false,
        error: Some(GeminiSettingsJsonValidationError {
...
fn parse_gemini_settings_json_raw(
    json: &str,
) -> Result<serde_json::Value, GeminiSettingsJsonValidationResult> {
    if json.len() > GEMINI_CONFIG_MAX_BYTES {
        return Err(json_validation_error(format!(
            "settings.json too large (max {GEMINI_CONFIG_MAX_BYTES} bytes)"
        )));
    }
...
pub fn validate_gemini_settings_json_raw(
    json: String,
) -> crate::shared::error::AppResult<GeminiSettingsJsonValidationResult> {
    Ok(match parse_gemini_settings_json_raw(&json) {
        Ok(_) => GeminiSettingsJsonValidationResult {
            ok: true,
            error: None,
        },
        Err(result) => result,
...
pub fn normalize_gemini_settings_json_raw(
    json: String,
) -> crate::shared::error::AppResult<Vec<u8>> {
    match parse_gemini_settings_json_raw(&json) {
        Ok(root) => json_to_bytes(&root, "gemini/settings.json"),
        Err(result) => {
            let err = result.error.unwrap_or(GeminiSettingsJsonValidationError {
...
pub fn gemini_settings_json_get_raw<R: tauri::Runtime>(
    app: &tauri::AppHandle<R>,
) -> crate::shared::error::AppResult<GeminiSettingsJsonState> {
    let path = gemini_config_path(app)?;
    let bytes = read_optional_gemini_config_file(&path)?;
    let exists = bytes.is_some();
    let json = match bytes {
        Some(bytes) => String::from_utf8(bytes).map_err(|_| {
            "SEC_INVALID_INPUT: gemini settings.json must be valid UTF-8".to_string()
        })?,
        None => String::new(),
    };

    Ok(GeminiSettingsJsonState {
...
pub fn gemini_settings_json_set_raw<R: tauri::Runtime>(
    app: &tauri::AppHandle<R>,
    json: String,
) -> crate::shared::error::AppResult<GeminiSettingsJsonState> {
    let path = gemini_config_path(app)?;
    ...
    let bytes = normalize_gemini_settings_json_raw(json)?;
    let _ = write_file_atomic_if_changed(&path, &bytes)?;
    gemini_settings_json_get_raw(app)
}
...
mod tests {
    ...
    #[test]
    fn validate_gemini_settings_json_raw_rejects_non_object_json() {
        let result = validate_gemini_settings_json_raw("[]".to_string()).expect("validation");
    ...
    #[test]
    fn normalize_gemini_settings_json_raw_pretty_prints_object() {
        let bytes = normalize_gemini_settings_json_raw(
        ...
        assert!(rendered.contains("\"name\": \"gemini-2.5-pro\""));
    }
}
