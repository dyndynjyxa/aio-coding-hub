...
export function createProviderEditorDialogSchema(options: { mode: "create" | "edit" }) {
  return z
    .object({
      ...
      oauth_provider_type: z.string().optional(),
      api_key: z.string(),
    ...
    .superRefine((values, ctx) => {
      if (values.auth_type === "oauth") {
        if (options.mode === "create" && !values.oauth_provider_type?.trim()) {
          ctx.addIssue({
            code: z.ZodIssueCode.custom,
            path: ["oauth_provider_type"],
            message: "OAuth 类型不能为空",
          });
        }
        return;
      }
